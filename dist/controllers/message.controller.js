...
export const getConversationList = async (req, res) => {
    ...
    try {
        ...
        const conversationArrayWithUnreadNumber = await Promise.all(conversations.map(async (conversation) => {
            const unreadCount = await Message.countDocuments({
                conversationId: conversation._id,
                readCheck: false,
            });
            const lastMessage = await Message.find({ conversationId: conversation._id }, { text: 1, createdAt: 1 })
                .sort({ createdAt: -1 })
                .limit(1);
            let newobject = {
                conversation,
...
export const getMessages = async (req, res) => {
    ...
    try {
        ...
        const formattedMessages = messages.map((message) => {
            const isSender = message.senderId._id?.toString() === userId.toString();
...
export const readMessages = async (req, res) => {
    const conversationId = req.params.conversationId;
    try {
        await Message.updateMany({ conversationId, readCheck: false }, { $set: { readCheck: true } });
...
export const payToUnlockImage = async (req, res) => {
